...
**Domain:** `Legal / Consumer Rights` · **Suite:** `Legal & Compliance Suite` · **Accent:** `#c084fc`
...
**Tos Plain English** is a standalone, browser-based AI workflow app for **Legal / Consumer Rights**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
|---|---:|:---:|---|---|
| `terms_of_service`<br>Terms of service | text | Yes | — | Affects input: Terms of service. |
...
| `user_type`<br>user type | text | No | — | Affects customization: user type. |
| `reading_level`<br>reading level | select | No | — | Affects customization: reading level. |
| `strictness`<br>strictness | slider | No | 50 | Affects customization: strictness. |
| `risk_focus`<br>risk focus | text | No | — | Affects customization: risk focus. |
| `evidence_quotes`<br>evidence quotes | text | No | — | Affects customization: evidence quotes. |
| `summary_length`<br>summary length | text | No | — | Affects customization: summary length. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9161
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9161`. |
...
```bash
curl http://127.0.0.1:9161/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9161/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9161 uvicorn app.main:app --host 0.0.0.0 --port 9161
```
...
- `plugins/tos-plain-english.json` for plugin metadata.
